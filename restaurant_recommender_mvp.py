...
with tab1:
    ...
    if ranked.empty:
        ...
        top_n = st.slider("Number of results", 3, min(20, len(ranked)), min(10, len(ranked)))
        st.dataframe(ranked[show_cols].head(top_n), use_container_width=True)

        st.subheader("Why these were recommended")
        explain = ranked.head(top_n)[["name", "content_score", "rating_score", "popularity_score", "distance_score", "budget_score", "open_score", "favorite_score", "final_score"]]
...
with tab3:
    ...
    if not ranked.empty:
        ...
        st.download_button(
            ...
            data=csv_data,
            file_name=f"recommendations_{selected_country}_{selected_city}.csv",
            mime="text/csv",
        )
        ...
        st.download_button(
            ...
            file_name="recommender_config.json",
            mime="application/json",
        )

st.markdown("---")
st.caption(
    "MVP note: this version uses cached or static restaurant data. The next production step is to add a data ingestion layer "
    "from Google Places or Foursquare and store refreshed restaurant data locally for cost control."
)
